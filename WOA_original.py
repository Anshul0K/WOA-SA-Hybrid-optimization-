...
# === Simple Whale Optimization Algorithm ===
def simple_woa(objective_function, lower_bound, upper_bound, max_evals, num_whales, dim):
    ...
    try:
        while eval_counter[0] < max_evals:
            ...
            for i in range(num_whales):
                ...
                else:  # Exploitation (spiral updating) - original WOA often defaults to spiral here
                    distance = np.abs(best_whale - whales[i])
                    whales[i] = distance * np.exp(b * l[i]) * np.cos(2 * np.pi * l[i]) + best_whale
...
def run_cec_benchmark_simple_woa(cec_funcs, year="CEC-2020", dim=30, runs=50, evals=1200, num_whales=30, output_dir="cec_simple_woa_results"):
    os.makedirs(output_dir, exist_ok=True)
    results_dict = {}

    for func_class in cec_funcs:
        func_name = func_class.__name__
        print(f"Running Simple WOA on {func_name} from {year} with dimension {dim}...")
        ...
        for i in range(runs):
            try:
                result = simple_woa(func_obj.evaluate, lower, upper, evals, num_whales, dim)
    ...
    csv_path = os.path.join(output_dir, f'{year}_dim_{dim}_simple_woa_results.csv')
    df.to_csv(csv_path)
    print(f"Saved: {csv_path}")


# === Run CEC Benchmarks with Simple WOA ===
if __name__ == "__main__":
    ...
    run_cec_benchmark_simple_woa(opfunu.get_functions_based_classname("2014"), year="CEC-2014", dim=common_dim, runs=50, evals=max_evaluations, num_whales=num_whales, output_dir="cec_simple_woa_results")
    run_cec_benchmark_simple_woa(opfunu.get_functions_based_classname("2017"), year="CEC-2017", dim=common_dim, runs=50, evals=max_evaluations, num_whales=num_whales, output_dir="cec_simple_woa_results")
    run_cec_benchmark_simple_woa(opfunu.get_functions_based_classname("2020"), year="CEC-2020", dim=common_dim, runs=50, evals=max_evaluations, num_whales=num_whales, output_dir="cec_simple_woa_results")
    run_cec_benchmark_simple_woa(opfunu.get_functions_based_classname("2022"), year="CEC-2022", dim=cec2022_dim, runs=50, evals=max_evaluations, num_whales=num_whales, output_dir="cec_simple_woa_results")

    print("Simple WOA benchmark runs completed.")