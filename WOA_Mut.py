...
# === Whale Optimization Algorithm with Enhanced Mutation ===
def woa_with_mutation(objective_function, lower_bound, upper_bound, max_evals, num_whales, dim, mutation_rate=0.05, mutation_scale=0.1):
            ...
            for i in range(num_whales):
                ...
                if p[i] < 0.5:
                    ...
                    if np.random.rand() < mutation_rate:
                        ...
                        mutation_amount = np.random.normal(0, mutation_scale * (upper_bound - lower_bound))
                        whales[i, mutation_dim] = np.clip(whales[i, mutation_dim] + mutation_amount, lower_bound, upper_bound)
...
def run_cec_benchmark_mutation(cec_funcs, year="CEC-2020", dim=30, runs=50, evals=1200, num_whales=30, output_dir="cec_mutation_results"):
    os.makedirs(output_dir, exist_ok=True)
    results_dict = {}

    for func_class in cec_funcs:
        func_name = func_class.__name__
        print(f"Running WOA with Mutation on {func_name} from {year} with dimension {dim}...")
        ...
        for i in range(runs):
            try:
                result = woa_with_mutation(func_obj.evaluate, lower, upper, evals, num_whales, dim)
    ...
    csv_path = os.path.join(output_dir, f'{year}_dim_{dim}_mutation_woa_results.csv')
    df.to_csv(csv_path)
    print(f"Saved: {csv_path}")


# === Run CEC Benchmarks with WOA and Enhanced Mutation ===
if __name__ == "__main__":
    common_dim = 30
    cec2022_dim = 20
    num_whales = 30
    max_evaluations = 1200 * 5
    mutation_rate = 0.1  # Adjust the mutation rate
    mutation_scale = 0.05 # Adjust the scale of mutation

    run_cec_benchmark_mutation(opfunu.get_functions_based_classname("2014"), year="CEC-2014", dim=common_dim, runs=50, evals=max_evaluations, num_whales=num_whales, output_dir="cec_mutation_results")
    run_cec_benchmark_mutation(opfunu.get_functions_based_classname("2017"), year="CEC-2017", dim=common_dim, runs=50, evals=max_evaluations, num_whales=num_whales, output_dir="cec_mutation_results")
    run_cec_benchmark_mutation(opfunu.get_functions_based_classname("2020"), year="CEC-2020", dim=common_dim, runs=50, evals=max_evaluations, num_whales=num_whales, output_dir="cec_mutation_results")
    run_cec_benchmark_mutation(opfunu.get_functions_based_classname("2022"), year="CEC-2022", dim=cec2022_dim, runs=50, evals=max_evaluations, num_whales=num_whales, output_dir="cec_mutation_results")

    print("WOA with Enhanced Mutation benchmark runs completed.")